...
module.exports.retrieveUser = async (req, res, next) => {
	const { username, offset, stepSize } = req.params;
	const requestingUser = res.locals.user;
	try {
		const user = await User.findOne({ username }, "username fullName avatar header bio bookmarks fullName _id website");
		if (!user) {
			return res.status(404).send({ error: "Could not find a user with that username." });
		}
		...
		const posts = await Post.aggregate([
			{
				$facet: {
					data: [
						...
						{ $sort: { date: -1 } },
						{
							$skip: Number(offset * stepSize),
						},
						{ $limit: Number(stepSize) },
						{
							$sample: { size: Number(stepSize) },
						},
						{
							$lookup: {
								from: "postvotes",
					...
					postCount: [{ $match: { author: ObjectId(user._id) } }, { $count: "postCount" }],
				},
			},
			{ $unwind: "$postCount" },
			{
		...
		return res.send({
			...
			isFollowing: requestingUser
				? !!followersDocument.followers.find((follower) => String(follower.user) === String(requestingUser._id))
				: false,
			posts: posts[0],
		});
	} catch (err) {
...
const retrieveRelatedUsers = async (user, userId, offset, followers) => {
	const pipeline = [
		...
		},
		{
			$lookup: {
				from: "users",
				let: followers ? { userId: "$followers.user" } : { userId: "$following.user" },
				pipeline: [
					{
						$match: {
	...
	const aggregation = followers ? await Followers.aggregate(pipeline) : await Following.aggregate(pipeline);
	...
	aggregation[0].userFollowers.forEach((followingUser) => {
		if (!!followingUser.followers.find((follower) => String(follower.user) === String(user._id))) {
...
module.exports.searchUsers = async (req, res, next) => {
	const { username, offset = 0, stepSize = 10 } = req.params;
	if (!username) {
...
module.exports.changeAvatar = async (req, res, next) => {
	...
	try {
		...
		const avatarUpdate = await User.updateOne({ _id: user._id }, { avatar: response.secure_url });
...
module.exports.changeHeader = async (req, res, next) => {
	const user = res.locals.user;
	...
	try {
		...
		const headerUpdate = await User.updateOne({ _id: user._id }, { header: response.secure_url });
		if (!headerUpdate.nModified) {
			throw new Error("Could not update user header.");
		}

		return res.send({ header: response.secure_url });
	} catch (err) {
		next(err);
	}
};
...
module.exports.removeAvatar = async (req, res, next) => {
	...
	try {
		const avatarUpdate = await User.updateOne({ _id: user._id }, { $unset: { avatar: "" } });
		if (!avatarUpdate.nModified) {
			next(err);
		}
	...
	try {
		...
		if (username) {
			...
			if (username !== user.username) {
				...
				if (existingUser) return res.status(400).send({ error: "Please choose another username." });
		...
		if (email) {
			...
			if (email !== user.email) {
				...
				if (existingUser) return res.status(400).send({ error: "Please choose another email." });
		...
		if (email && email !== user.email) {
			sendConfirmationEmail(updatedUser.username, updatedUser.email, confirmationToken.token);
		}
	} catch (err) {
		next(err);
	}
