...
const { sendConfirmationEmail, generateUniqueUsername } = require("../utils/controllerUtils");
...
module.exports.verifyJwt = (token) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			const user = await User.findOne({ _id: id }, "email username avatar bookmarks bio fullName confirmed website");
			if (user) {
				return resolve(user);
			} else {
...
module.exports.loginAuthentication = async (req, res, next) => {
	...
	if (!usernameOrEmail || !password) {
		return res.status(400).send({ error: "Please provide both a username/email and a password." });
	}
...
module.exports.register = async (req, res, next) => {
	...
	sendConfirmationEmail(userName[0], user.email, confirmationToken.token);
};
		...
		if (existingUser) {
			if (existingUser.email === primaryEmail) {
				return res.status(400).send({
					error: "A user with the same email already exists, please change your primary github email.",
				});
			}
