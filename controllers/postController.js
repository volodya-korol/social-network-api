...
const { retrieveComments, formatCloudinaryUrl, populatePostsPipeline } = require("../utils/controllerUtils");
...
module.exports.createPost = async (req, res, next) => {
	...
	try {
		...
		if (moderationResponse.data.error) {
			return res.status(500).send({ error: "Error moderating image, please try again later." });
		}
...
module.exports.deletePost = async (req, res, next) => {
	...
	try {
		...
		followers.forEach((follower) => socketHandler.deletePost(req, postId, follower.user));
	} catch (err) {
		console.log(err);
	}
};
...
module.exports.retrievePost = async (req, res, next) => {
	...
	try {
		...
		const post = await Post.aggregate([
			...
			{ $unwind: "$postVotes" },
			{
				$unset: ["author.password", "author.email", "author.private", "author.bio", "author.githubId"],
			},
			{
...
module.exports.votePost = async (req, res, next) => {
	...
	try {
		...
		if (!postLikeUpdate.nModified) {
			...
			const postDislikeUpdate = await PostVote.updateOne({ post: postId }, { $pull: { votes: { author: user._id } } });
...
module.exports.retrievePostFeed = async (req, res, next) => {
	const user = res.locals.user;
	const { offset, stepSize } = req.params;
...
module.exports.retrieveSuggestedPosts = async (req, res, next) => {
	const { offset = 0, stepSize = 10 } = req.params;
